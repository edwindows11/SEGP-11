...
const DESCRIPTIONS := {
	...
	VILLAGE_HEAD:          "May play up to 2 coloured cards per turn (only 1 villager-increasing).",
...
static func max_cards_per_turn(role: String) -> int:
	if role == VILLAGE_HEAD:
		return 2
	if role == ENVIRONMENTAL_CONSULT and GameState.ec_borrowed_ability == VILLAGE_HEAD:
		return 2
	return 1

# --- Goal tracker display data ----------------------------------------------
# Per-role panel styling. The UI builds N generic player tracker panels at
# startup and pulls the title/border colour from here so styling stays close
# to the original per-role panels.
const GOAL_DISPLAY := {

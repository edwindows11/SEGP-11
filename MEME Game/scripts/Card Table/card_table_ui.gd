...
func hide_instruction(): instruction_label.get_parent().visible = false
...
func show_player_select_popup(title_text, disabled_func, callback):
	...
	for i in range(GameState.player_count):
		...
		btn.pressed.connect(func(): steal_popup.visible = false; callback.call(i))
		vbox.add_child(btn)
	steal_popup.visible = true

func _on_po_ability_btn_pressed(): if cards_played_this_turn == 0: request_po_ability.emit()
func _on_gov_ability_btn_pressed(): if not gov_used_ability_this_turn: request_gov_ability.emit()
func _on_cons_ability_btn_pressed(): if not cons_used_ability_this_turn: request_cons_ability.emit()
func _on_ld_ability_btn_pressed(): if not ld_used_ability_this_turn: request_ld_ability.emit()
func _on_ec_ability_btn_pressed(): request_ec_ability.emit()
...
func show_em_choice_popup(callback):
	...
	for opt in [{"t":"🐘 Elephant","v":"elephant"},{"t":"🧑 Villager","v":"villager"},{"t":"Skip","v":"skip"}]:
		...
		b.pressed.connect(func(): em_choice_popup.visible=false; callback.call(opt.v))
		vbox.add_child(b)
...
func show_convert_type_popup(card_effects: Node, current_type: int) -> void:
	...
	for btn in row_node.get_children():
		if btn is Button:
			match btn.name:
				"ForestBtn": btn.disabled = (current_type == GameState.TileType.FOREST)
				"HumanBtn": btn.disabled = (current_type == GameState.TileType.HUMAN)
				"PlantationBtn": btn.disabled = (current_type == GameState.TileType.PLANTATION)

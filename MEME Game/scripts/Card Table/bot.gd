...
## Delay before the bot decides on a card (so the move feels human).
var think_delay: float = 2.0
## Delay after the card pops up before its effect starts resolving.
var card_reveal_delay: float = 1.4
## Delay between clicking tiles for multi-step effects.
var action_delay: float = 0.85
## Delay before actually ending the bot's turn.
var end_turn_delay: float = 1.8

## Queue of tile keys the bot plans to click next. Drained by _process.
var _pending_selections: Array = []
## Countdown until the next queued selection is sent.
var _action_timer: float = 0.0
## True while a selection is queued but not yet sent.
...
## The player_index whose turn is currently being played.
var _current_bot_player: int = -1
## True once the Ecotourism Manager already used its extra-move this turn.
...
func _on_turn_changed(player_index: int, _role_name: String, is_skipped: bool) -> void:
	...
	_vh_activated = false
	_vh_first_card_added_villagers = false
	_vh_first_card_id = ""
...
func _bot_take_turn(player_index: int) -> void:
	...
	if ability_role == "Village Head" and hand.size() >= 3:
		_vh_activated = true
		ui.vh_used_ability_this_turn = true
		_announce_bot_message(player_index, "uses Village Head Special Ability!", true)
	...
	# Track if this card adds villagers (for Village Head 2nd-card constraint)
	if _vh_activated and not _vh_first_card_added_villagers:
		for fx in card_def.get("sub_effects", []):
			if fx.get("op", "") in ["add_v", "add_v_in"]:
				_vh_first_card_added_villagers = true
				break
	...
	match ability_role:
		...
		"Plantation Owner":
			...
			if best_target != -1 and randf() > 0.3:
				...
				if not card_effects.effects_complete.is_connected(_on_bot_effects_complete):
					card_effects.effects_complete.connect(_on_bot_effects_complete)
				# Reversed cards may include interactive ops (move, convert, etc.),
				# so the bot's tile-click responder must be wired up too — otherwise
				# the effect stalls in WAITING_SOURCE with no clicker.
				if not card_effects.request_tile_selection.is_connected(_on_bot_tile_selection_requested):
					card_effects.request_tile_selection.connect(_on_bot_tile_selection_requested)
...
var _vh_first_card_id: String = ""  # Village Head 1st card (tracked separately for stats)
...
func _role_bonus_medium(card_id: String, role: String) -> float:
	...
	for fx in effects:
		...
		match role:
			...
			"Wildlife Department":
				if op == "add_e": bonus += 4.0
				if op == "convert" and fx.get("to","") == "FOREST": bonus += 4.0
				if op == "move_e" and fx.get("to","") == "FOREST": bonus += 3.0
			"Ecotourism Manager":
...
func _score_card_hard(card_id: String, role: String, player_index: int, snap: Dictionary = {}) -> float:
	...
	if role == "Wildlife Department":
		...
		var wd_green_deficit: int = maxi(0, 4 - wd_green_played)
		if wd_green_deficit > 0 and color == Color.GREEN:
			score += 5.0 * wd_green_deficit  # up to +20 when at 0/4
	...
	for fx in effects:
		...
		match role:
			...
			"Wildlife Department":
				...
				if op == "add_e":
					bonus += 8.0 * min(count, wd_e_deficit)
					if forest_count > 0:
						bonus += 3.0  # elephants more useful when forests exist
				# Reward converting tiles to forest (elephants need forest)
				if op == "convert" and fx.get("to","") == "FOREST":
					bonus += 6.0 * count
					if wd_e_deficit > 0:
						bonus += 4.0  # more forest helps place elephants
				# Reward moving elephants into forest
				if op == "move_e":
					var to_type = fx.get("to", "ANY")
					if to_type == "FOREST":
						bonus += 8.0 * min(count, wd_e_deficit)
...
## Picks "elephant" or "villager" with 50/50 odds for the Ecotourism
## Manager choice popup.
...
func _on_bot_effects_complete() -> void:
	...
	if ability_role == "Village Head" and _vh_activated and ui.cards_played_this_turn == 1 and card_effects.state == 0:
		...
		if hand.size() >= 2:
			# Filter to coloured cards only; if 1st card added villagers, exclude cards that add villagers
			var valid_2nd: Array = []
			for cid in hand:
				var cdata = CardData.ALL_CARDS.get(cid, {})
				var col = cdata.get("color", Color.WHITE)
				if not col in [Color.GREEN, Color.YELLOW, Color.RED]:
					continue
				if _vh_first_card_added_villagers:
					var adds_v = false
					for fx in cdata.get("sub_effects", []):
						if fx.get("op", "") in ["add_v", "add_v_in"]:
							adds_v = true
							break
					if adds_v:
						continue
				valid_2nd.append(cid)

			if not valid_2nd.is_empty():
				var difficulty: Difficulty = bot_players.get(_current_bot_player, Difficulty.EASY)
				var chosen_2nd: String = ""
				match difficulty:
					Difficulty.EASY:
						valid_2nd.shuffle()
						chosen_2nd = valid_2nd[0]
					Difficulty.MEDIUM:
						chosen_2nd = _pick_card_medium(valid_2nd, _current_bot_player)
						if chosen_2nd == "": chosen_2nd = valid_2nd[0]
					Difficulty.HARD:
						chosen_2nd = _pick_card_hard(valid_2nd, _current_bot_player)
						if chosen_2nd == "": chosen_2nd = valid_2nd[0]

				var cdef = CardData.ALL_CARDS.get(chosen_2nd, {})
				_announce_bot_message(_current_bot_player, "plays 2nd card: " + str(cdef.get("name", chosen_2nd)), true)
				await get_tree().create_timer(card_reveal_delay, false).timeout
				if not _bot_is_acting: return

				_bot_played_card_id = chosen_2nd
				ui.cards_played_this_turn += 1
				if not card_effects.effects_complete.is_connected(_on_bot_effects_complete):
					card_effects.effects_complete.connect(_on_bot_effects_complete)
				if not card_effects.request_tile_selection.is_connected(_on_bot_tile_selection_requested):
					card_effects.request_tile_selection.connect(_on_bot_tile_selection_requested)
				card_effects.execute_card(chosen_2nd)
				return
...
## Returns the tile key in `keys` with the largest distance to any HUMAN
## tile. Used to move elephants somewhere safer.
...
## Returns the tile key in `keys` that borders the most forest tiles.
## Used when the bot wants to expand forest most efficiently.
...
## Returns the tile key in `keys` with the fewest useful neighbours.
## Used to pick tiles that are cheap to give up when converting.
...
## Shortest Manhattan distance from `origin` to any tile of `tile_type`.
## Used by the human-distance helpers above.
...
## Returns the four up/down/left/right neighbour tile keys of `key`.
...
## Counts every elephant currently on the board.
